...
- 📅 **Розклад** — тижневий/місячний календар
- 📋 **Список** — timeline з фокусом на сьогодні
...
- `renderLessons()` — розклад (тиждень/місяць)
- `renderTimeline()` — загальний список
...
- `jumpToDay(dateStr)` — перехід до дня з timeline
